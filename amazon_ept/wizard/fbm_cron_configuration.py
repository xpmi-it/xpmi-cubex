...
IMPORT_FBM_ORDER = 'amazon_ept.ir_cron_import_missing_unshipped_orders_seller_%d'
...
class FbmCronConfiguration(models.TransientModel):
    ...
    amz_order_auto_import = fields.Boolean(string='Auto Import FBM Un Shipped Orders ?')
    ...
    def save_cron_configuration(self):
        ...
        self.setup_amz_unshipped_order_cron(amazon_seller)
    ...
    # commented below both methods because import FBM unshipped orders report is deprecated in v17
    ...
    def setup_amz_unshipped_order_cron(self, amazon_seller):
        """
        This method will set up the auto import missing unshipped order scheduler
        ...
        if self.amz_order_auto_import and self.amazon_selling not in ['FBA']:
            ...
            if cron_exist:
                ...
                import_order_cron = self.env.ref('amazon_ept.ir_cron_import_missing_unshipped_orders',
                                                 raise_if_not_found=False)
                if not import_order_cron:
                    raise UserError(_('Core settings of Amazon import unshipped orders '
                                      'are deleted, please upgrade Amazon module to back this settings.'))
                name = 'FBM-' + amazon_seller.name + ' : Amazon Import Unshipped Orders'
                vals.update({'name': name})
                new_cron = import_order_cron.copy(default=vals)
    ...
    # commented below both methods because process FBM unshipped orders report is deprecated in v17
