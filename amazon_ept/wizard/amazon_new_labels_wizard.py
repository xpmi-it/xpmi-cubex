    ...
    def get_labels(self):
        ...
        if shipment_id and model == AMAZON_INBOUND_SHIPMENT_EPT:
            ...
            for box_no in list_box_no:
                ...
                if shipment_rec.is_ltl_shipment:
                    break
            msg = "Amazon Labels Downloaded"
            if ctx.get('label_type', '') == 'delivery':
                shipment_rec.write({'is_pallet_label_downloaded': True})
                msg = "Amazon Pallet labels Downloaded"
            shipment_rec.message_post(body=_(msg), attachment_ids=labels_list)
