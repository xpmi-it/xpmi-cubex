...
class FeedSubmissionHistory(models.Model):
    ...
    def get_feed_submission_result(self):
        ...
        merchant_id = self.seller_id.merchant_id if self.seller_id and self.seller_id.merchant_id else False
    ...
    def export_invoice_check_feed_status_cron(self):
        """
        Purpose: The scheduler to Check the status of feed for export invoice in Amazon.
        :return:
        """
        common_log_obj = self.env['common.log.lines.ept']
        export_invoice_feeds = self.search([('feed_type', '=', 'upload_invoice'), ('feed_result', '=', False)],
                                           limit=10)
        for export_invoice_feed in export_invoice_feeds:
            invoice = export_invoice_feed.invoice_id
            export_invoice_feed_response = export_invoice_feed.with_context(
                auto_process=True).get_feed_submission_result()
            if export_invoice_feed_response and export_invoice_feed_response.__contains__('Error'):
                if (export_invoice_feed_response.__contains__(
                        "Amazon will generate invoice")):
                    continue
                if (export_invoice_feed_response.__contains__(
                        "already a document with same invoice")):
                    continue
                else:
                    message = 'While check the Feed Result Found issue In Feed Result. Please check Feed: %s and invoice is %s:' % (
                        export_invoice_feed.id, invoice.name)
                    common_log_obj.create_common_log_line_ept(message=message,
                                                              model_name='feed.submission.history',
                                                              module='amazon_ept', fulfillment_by='',
                                                              amz_seller_ept=self.seller_id.id if
                                                              self.seller_id.id else False)
                    invoice.write({'invoice_sent': False, 'feed_id': False})
