...
class ResPartner(models.Model):
    ...
    # _search(self, domain, offset=0, limit=None, order=None):
    @api.model
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        if not self.env.context.get('is_amazon_partner', True):
            domain = [('is_amz_customer', '=', False)] + list(domain)
        return super(ResPartner, self)._search(domain, offset, limit, order)
    ...
    def auto_delete_customer_pii_details(self):
        ...
        if seller_ids:
            ...
            query = sql.SQL("""update res_partner set name=concat('Amazon-',T.sale_name),commercial_company_name='Amazon', 
                                ...
                                and res_partner.id {}""").format(sql.SQL(partner_to_skip))
