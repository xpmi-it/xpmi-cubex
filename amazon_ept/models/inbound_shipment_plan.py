        ...
        for shipment in odoo_shipments:
            ...
            if not warehouse:
                error_value = ('Warehouse not found for Fulfillment Center: %s during inbound shipment %s.\n'
                               ...
                               '-Set the warehouse and re-import the shipment using the operation wizard.') % (fulfill_center, shipment.name)
            ...
            if not location_routes:
                error_value = ('Cannot generate transfer, routes not found for warehouse: %s (Shipment: %s).\n'
                               ...
                               '-Update routing configuration and re-import the shipment.') % (warehouse.name, shipment.name)
                log_line_obj.create_common_log_line_ept(
                    message=error_value, model_name=AMAZON_INBOUND_SHIPMENT_EPT, module='amazon_ept',
                    operation_type='export', res_id=shipment.id,
                    amz_instance_ept=ship_plan.instance_id and ship_plan.instance_id.id or False,
