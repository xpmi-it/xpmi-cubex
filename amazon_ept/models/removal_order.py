...
class RemovalOrder(models.Model):
    ...
    def create_removal_order(self):
        ...
        fl.seek(0)
        new_data = base64.b64encode(fl.read()).decode('utf-8')
    ...
    def disposal_order_pickings(self):
        ...
        if not config:
            message = ("Removal order configuration for Disposition Type '{}' not found.\n"
                "Please follow these steps to resolve:\n"
                "1. Navigate to: Amazon → Configuration → Removal Orders.\n"
                "2. Configure the '{}' disposition type.\n"
                "3. Re-run the operation wizard to create the removal plan."
            ).format(self.removal_disposition, self.removal_disposition)
    ...
    def removal_order_procurements(self):
        ...
        unsellable_rule = config.unsellable_route_id.rule_ids.filtered(lambda l: l.sequence == require_stock_rule_seq)
