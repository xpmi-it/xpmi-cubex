...
class AmazonStockAdjustmentConfig(models.Model):
    ...
    def _compute_company(self):
        """
        This method will set the company for stock adjustment configuration.
        """
        for record in self:
            company_id = record.seller_id.company_id.id if record.seller_id else self.env['res.company']
            if not company_id:
