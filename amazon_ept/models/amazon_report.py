...
class AmazonReports(models.AbstractModel):
    ...
    def get_report(self):
        ...
        if self.report_document_id:
            ...
            if amz_report_type and self._description == 'Shipping Report':
                amz_report_type = '' if self.report_type == ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA_GENERAL.value else \
                    amz_report_type
    ...
    def amz_check_report_required_headers(self, required_header, report_headers, is_auto_process=False):
        """
        Define this method for check process report headers in English language or
        not.
        :param: required_header: report header - str
        :param: report_headers: list report headers - []
        :param: is_auto_process: True/False
        :return: User Warning or True or False
        """
        common_log_line_obj = self.env[COMMON_LOG_LINES_EPT]
        if required_header not in report_headers:
            self.write({'state': 'CANCELLED'})
            message = ("There is another language used in this report besides English. From Seller Central, "
                       "kindly change the report language to English. To change the language, follow the steps below.\n"
                       "1. Log in to your Seller Central account \n"
                       "2. Settings =>> Account Info \n"
                       "3. Under the Business Information Section \n"
                       "     3.1 Language for Feed Processing Reports")
